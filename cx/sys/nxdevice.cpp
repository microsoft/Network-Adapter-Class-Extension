...
{
    m_State = DeviceState::ReleasingPhase2;
    ...
    )
{
    ...
    CxPrePrepareHardwareFailedCleanupHandled.Set(STATUS_SUCCESS);
    return NxDevice::Event::SyncSuccess;
}
    ...
    )
{
    ...
    (void)nxDevice->CxPrePrepareHardwareFailedCleanupHandled.Wait();
}
...
--*/
{
    ...
    return nxDevice->CxPreSelfManagedIoSuspendHandled.Wait();
}
...
--*/
{
    ...
    {
        m_State = DeviceState::ReleasingPhase1;
    }

    auto ntStatus = m_AdapterCollection.ForEachAdapterLocked(
        [this](NxAdapter &adapter)
    {
        if (adapter.GetCurrentState() == AdapterState::Started)
        {
    ...
    CxPreSelfManagedIoSuspendHandled.Set(ntStatus);
...
--*/
{
    ...
    // Eventually we want to try this first if the client driver has registered with the callback.
    else if (m_EvtNetDeviceReset != NULL)
    {
        m_EvtNetDeviceReset(GetFxObject());
    }
    else
    {
        LogInfo(GetRecorderLog(), FLAG_DEVICE, "No Reset Operation was triggered");
    }
    return status;
}
