...
NDIS_PM_CAPABILITIES_INIT_FROM_NET_ADAPTER_POWER_CAPABILITIES(
    ...
    _In_ NET_ADAPTER_POWER_OFFLOAD_ARP_CAPABILITIES const * ArpCapabilities,
    _In_ NET_ADAPTER_POWER_OFFLOAD_NS_CAPABILITIES const * NSCapabilities,
    _In_ NET_ADAPTER_WAKE_BITMAP_CAPABILITIES const * BitmapCapabilities,
    _In_ NET_ADAPTER_WAKE_MAGIC_PACKET_CAPABILITIES const * MagicPacketCapabilities,
    _In_ NET_ADAPTER_WAKE_MEDIA_CHANGE_CAPABILITIES const * MediaChangeCapabilities,
    _In_ NET_ADAPTER_WAKE_PACKET_FILTER_CAPABILITIES const * PacketFilterCapabilities
)
{
    ...
    NdisPowerCaps->MediaSpecificWakeUpEvents = MediaSpecificWakeUpEvents;

    if (MediaChangeCapabilities->MediaConnect)
    {
        NdisPowerCaps->SupportedWakeUpEvents |= NDIS_PM_WAKE_ON_MEDIA_CONNECT_SUPPORTED;
    }

    if (MediaChangeCapabilities->MediaDisconnect)
    {
        NdisPowerCaps->SupportedWakeUpEvents |= NDIS_PM_WAKE_ON_MEDIA_DISCONNECT_SUPPORTED;
    }

    if (MagicPacketCapabilities->MagicPacket)
    {
        NdisPowerCaps->SupportedWoLPacketPatterns |= NDIS_PM_WOL_MAGIC_PACKET_SUPPORTED;
    }

    if (BitmapCapabilities->BitmapPattern)
    {
        NdisPowerCaps->SupportedWoLPacketPatterns |= NDIS_PM_WOL_BITMAP_PATTERN_SUPPORTED;
    }

    NdisPowerCaps->MaxWoLPatternSize = static_cast<ULONG>(BitmapCapabilities->MaximumPatternSize);
    NdisPowerCaps->NumTotalWoLPatterns = static_cast<ULONG>(BitmapCapabilities->MaximumPatternCount);

    if (PacketFilterCapabilities->PacketFilterMatch)
    {
        NdisPowerCaps->Flags |= NDIS_PM_SELECTIVE_SUSPEND_SUPPORTED;
    }

    if (ArpCapabilities->ArpOffload)
    {
        NdisPowerCaps->SupportedProtocolOffloads |= NDIS_PM_PROTOCOL_OFFLOAD_ARP_SUPPORTED;
        NdisPowerCaps->NumArpOffloadIPv4Addresses = static_cast<ULONG>(ArpCapabilities->MaximumOffloadCount);
    }

    if (NSCapabilities->NSOffload)
    {
        NdisPowerCaps->SupportedProtocolOffloads |= NDIS_PM_PROTOCOL_OFFLOAD_NS_SUPPORTED;
        NdisPowerCaps->NumNSOffloadIPv6Addresses = static_cast<ULONG>(NSCapabilities->MaximumOffloadCount);
    }
