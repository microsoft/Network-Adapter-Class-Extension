...
{
    //
    // TODO change this logic to use RAII
    //
...
)
{
    ...
    CX_RETURN_NTSTATUS_IF(STATUS_INSUFFICIENT_RESOURCES, !m_offloads.append(wistd::move(lsoOffload)));
...
)
{
    ...
    //
    // TODO: Illegal to set active capabilities on an offload which is not supported by client driver
    // Add some validation layer before setting active capabilities, to prevent activating sub-capability
    // not supported in hardware capabilities
    //
    ...
    if (offloadCallback)
    {
        offloadCallback(m_offloadFacade->GetNetAdapter(),
